// EFF short wordlist #1 — 1296 words, ~10.3 bits per random word.
// Source: https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt (CC BY 3.0 US).
// Baked in so passphrase generation runs fully offline, in the browser. Words are
// short, distinct, and memorable — ideal for a spoken/shared room secret.
...
// Fast membership test for typed passphrase words. Validation is LENIENT: words
// outside this set are allowed (custom passphrases work) but flagged in the UI so
// typos get caught. Lowercased + trimmed to match how both surfaces canonicalize.
export const WORDSET: ReadonlySet<string> = new Set(WORDLIST)
export function isWord(w: string): boolean {
  return WORDSET.has(w.trim().toLowerCase())
}

// A friendly, deterministic two-word label for a room, derived from the opaque
// room id (a KDF output). It is a hash-of-a-hash, so it never reveals the
// passphrase — and every client in the room computes the same label. Purely
// cosmetic (the room id is what identifies the room on the wire).
export function roomLabel(roomId: string): string {
  let a = 2166136261
  let b = 5381
  for (let i = 0; i < roomId.length; i++) {
    const c = roomId.charCodeAt(i)
    a = (Math.imul(a, 16777619) ^ c) >>> 0
    b = ((Math.imul(b, 33) + c) ^ (b >>> 5)) >>> 0
  }
  return `${WORDLIST[a % WORDLIST.length]}-${WORDLIST[b % WORDLIST.length]}`
}
